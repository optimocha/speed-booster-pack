...
class Tweaks extends AbstractModule {

	public function __construct() {

		parent::__construct();

		if ( ! Helper::getOption( 'module_tweaks' ) ) {
			return;
		}
	...
	private function trim_query_strings() {
		...
		if ( is_admin() || ! Helper::getOption( 'trim_query_strings' ) ) { return; }
	...
	private function dequeue_emoji_scripts() {
		...
		if ( ! Helper::getOption( 'dequeue_emoji_scripts' ) || is_admin() ) { return; }
	...
	private function disable_self_pingbacks() {
		...
		if ( ! Helper::getOption( 'disable_self_pingbacks' ) ) { return; }
	...
	private function dequeue_dashicons() {
		...
		if ( ! Helper::getOption( 'dequeue_dashicons' ) ) { return; }
	...
	private function post_revisions() {
		...
		$post_revisions = Helper::getOption( 'post_revisions' );
	...
	private function autosave_interval() {
		...
		$autosave_interval = Helper::getOption( 'autosave_interval' );

		if ( empty( $autosave_interval ) || ! (int) $autosave_interval || defined( 'AUTOSAVE_INTERVAL' ) ) { return; }
		
		define( 'AUTOSAVE_INTERVAL', Helper::getOption( 'autosave_interval' ) * 60 );

	}

	private function dequeue_block_library() {

		if ( ! Helper::getOption( 'dequeue_block_library' ) ) { return; }
	...
	private function dequeue_global_styles() {
		...
		if ( ! Helper::getOption( 'dequeue_global_styles' ) ) { return; }
	...
	private function disable_post_embeds() {
		...
		if ( ! Helper::getOption( 'disable_post_embeds' ) ) { return; }
	...
	private function heartbeat_settings() {
		...
		switch ( Helper::getOption( 'heartbeat_settings' ) ) {
	...
	private function instant_page() {
		...
		if ( ! Helper::getOption( 'instant_page' ) ) { return; }
	...
	private function dequeue_comment_reply_script() {
		...
		if ( ! Helper::getOption( 'dequeue_comment_reply_script' ) ) { return; }
	...
	private function declutter_shortlinks() {
		...
		if ( ! is_array( Helper::getOption( 'declutter_head' ) ) || ! Helper::getOption( 'declutter_head' )[ 'declutter_shortlinks' ] ) { return; }
	...
	private function declutter_adjacent_posts_links() {
		...
		if ( ! is_array( Helper::getOption( 'declutter_head' ) ) ||  ! Helper::getOption( 'declutter_head' )[ 'declutter_adjacent_posts_links' ] ) { return; }
	...
	private function declutter_wlw() {
		...
		if ( ! is_array( Helper::getOption( 'declutter_head' ) ) ||  ! Helper::getOption( 'declutter_head' )[ 'declutter_wlw' ] ) { return; }
	...
	private function declutter_rsd() {
		...
		if ( ! is_array( Helper::getOption( 'declutter_head' ) ) ||  ! Helper::getOption( 'declutter_head' )[ 'declutter_rsd' ] ) { return; }
	...
	private function declutter_rest_api_links() {
		...
		if ( ! is_array( Helper::getOption( 'declutter_head' ) ) ||  ! Helper::getOption( 'declutter_head' )[ 'declutter_rest_api_links' ] ) { return; }
	...
	private function declutter_feed_links() {
		...
		if ( ! is_array( Helper::getOption( 'declutter_head' ) ) ||  ! Helper::getOption( 'declutter_head' )[ 'declutter_feed_links' ] ) { return; }
	...
	private function declutter_wp_version() {
		...
		if ( ! is_array( Helper::getOption( 'declutter_head' ) ) ||  ! Helper::getOption( 'declutter_head' )[ 'declutter_wp_version' ] ) { return; }
