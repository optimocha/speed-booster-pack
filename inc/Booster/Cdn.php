...
class Cdn extends AbstractModule {
	...
	public function __construct() {
		...
		if ( ! Helper::getOption( 'cdn_url' ) || ! Helper::getOption( 'cdn_enable' ) ) {
			return;
		}
	...
	private function set_included_dirs() {
		...
		$includes            = Helper::getOption( 'cdn_includes' );
		$lines               = Helper::explodeLines( $includes, true );
	...
	private function set_excluded_extensions() {
		$excludes                  = Helper::getOption( 'cdn_excludes' );
		$lines                     = Helper::explodeLines( $excludes, true );
	...
	private function replace_url( $url ) {
		$cdn_url = '//' . Helper::getOption( 'cdn_url' );
...
//		}
	...
	private function is_excluded( $url ) {
		$extension = Helper::get_file_extension_from_url( $url );
