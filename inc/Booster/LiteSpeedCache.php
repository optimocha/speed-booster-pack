...
class LiteSpeedCache  {
	...
	public function add_admin_bar_links( \WP_Admin_Bar $admin_bar ) {
		if ( current_user_can( 'manage_options' ) ) {
			...
			if ( Helper::getOption( 'module_caching_ls' ) ) {
	...
	public static function insert_htaccess_rules() {
		...
		if ( Helper::getOption( 'module_caching_ls' ) ) {
			...
			if ( Helper::getOption( 'caching_ls_cache_logged_in_users' ) ) {
			...
			if ( Helper::getOption( 'caching_ls_separate_mobile' ) ) {
			...
			if ( $query_strings = Helper::getOption( 'caching_ls_exclude_cookies' ) ) {
			...
			if ( $query_strings = Helper::getOption( 'caching_ls_include_query_strings' ) ) {
				$keys = explode( PHP_EOL, $query_strings );
				if ( $keys ) {
		...
		Helper::insertToHtaccess( self::ROOT_MARKER, implode( PHP_EOL, $lines ) );
	}

	public static function remove_htaccess_rules() {
		Helper::insertToHtaccess( self::ROOT_MARKER, '' );
	}
	...
	private function add_tags() {
		...
		if ( Helper::getOption( 'caching_ls_separate_mobile' ) && wp_is_mobile() ) {
			$tags[] = 'is_mobile';
		}
	...
	public function set_headers() {
		if ( ! Helper::getOption( 'module_caching_ls' ) ) {
			...
			$cache_expire_time = Helper::getOption( 'caching_ls_expiry', 10 ) * HOUR_IN_SECONDS;

			// Check for all exclusions
			if ( true === $this->should_bypass_cache( [ 'is_logged_in', 'include_query_strings' ] ) ) {
				$this->headers['X-LiteSpeed-Cache-Control'] = 'no-cache';
			} else {
				if ( ! Helper::getOption( 'caching_ls_cache_logged_in_users' ) && is_user_logged_in() ) {
					$this->headers['X-LiteSpeed-Cache-Control'] = 'no-cache';
				} else {
					$this->add_tags();
...
//			add_action( 'edit_comment', [ $this, 'clear_post_by_comment' ] );
	...
	public function add_cache_signature( $html ) {
		if ( Helper::isLitespeed() && Helper::getOption( 'module_caching_ls' ) ) {
