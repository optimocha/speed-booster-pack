...
class BaseCache  {
	...
	public function __construct() {
		$this->is_litespeed = Helper::isLiteSpeed();
	}
	...
	private function check_excluded_urls() {
		...
		if ( $exclude_urls = Helper::getOption( 'caching_' . ( $this->is_litespeed !== false ? 'ls_' : '' ) . 'exclude_urls' ) ) {
			$exclude_urls   = array_map( 'trim', Helper::explodeLines( $exclude_urls ) );
	...
	private function check_cookies() {
		...
		if ( ! empty( $_COOKIE ) ) {
			...
			$excluded_cookies = Helper::getOption( 'caching_' . ( $this->is_litespeed !== false ? 'ls_' : '' ) . 'exclude_cookies' );
			$excluded_cookies = Helper::explodeLines( $excluded_cookies );
