...
class Woocommerce extends AbstractModule {
	public function __construct() {
		...
		if ( ! Helper::getOption( 'module_woocommerce' ) ) {
			return;
		}
	...
	public function run_class() {
		...
		if ( ! $this->should_sbp_run || ! Helper::isPluginActive( 'woocommerce/woocommerce.php' ) ) {
			return;
		}
	...
	public function optimize_nonwc_pages_handle() {
		if ( Helper::getOption( 'woocommerce_optimize_nonwc_pages' ) ) {
	...
	private function woocommerce_disable_cart_fragments() {
		if ( Helper::getOption( 'woocommerce_disable_cart_fragments' ) ) {
	...
	private function remove_wc_password_strength_meter() {
		if ( function_exists( 'is_account_page' ) && Helper::getOption( 'woocommerce_disable_password_meter' ) ) {
	...
	public function remove_marketing() {
		if ( ! Helper::getOption( 'woocommerce_marketing' ) ) {
	...
	private function set_action_scheduler_period() {
		add_filter( 'action_scheduler_retention_period', function () {
			return DAY_IN_SECONDS * Helper::getOption( 'woocommerce_action_scheduler_period', 7 );
		} );
	}
