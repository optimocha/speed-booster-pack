...
class CssMinifier extends AbstractModule {
	private $styles_list = [];
	private $exceptions = [
		'admin-bar',
		'dashicons',
	...
	public function __construct() {
		...
		if ( ! Helper::getOption( 'module_css' ) || ! Helper::getOption( 'css_inline' ) || Helper::getOption( 'enable_criticalcss' ) ) {
			return;
		}
	...
	public function print_styles( $html ) {
		if ( Helper::getOption( 'css_minify' ) ) {
			$minify = true;
		} else {
			$minify = false;
		}
	...
	private function set_exceptions() {
		$sbp_exceptions   = Helper::explodeLines( Helper::getOption( 'css_exclude' ) );
	...
	private function inline_css( $url, $minify = true ) {
		...
		$url    = Helper::clear_hashes_and_question_mark( $url );
