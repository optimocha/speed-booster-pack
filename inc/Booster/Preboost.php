...
class Preboost extends AbstractModule {
	...
	public function __construct() {
		...
		if ( is_array( Helper::getOption( 'preboost' ) ) && ( ! Helper::getOption( 'module_assets' ) ) ) {
			return;
		}
	...
	public function run_class() {
		if ( $this->should_sbp_run ) {
			$preboost = Helper::getOption( 'preboost' );
	...
	private function prepare_preload_tags() {
		$urls = [];
		if ( isset( Helper::getOption( 'preboost' )['preboost_include'] ) ) {
			$urls = Helper::explodeLines( Helper::getOption( 'preboost' )['preboost_include'] );
		}

		if ( is_singular() ) {
			$content_specific_preload_rules = Helper::sbpGetPostMeta( get_the_ID(), 'sbp_preload' );
			if ( $content_specific_preload_rules !== null ) {
				$content_specific_preload_rules_array = Helper::explodeLines( $content_specific_preload_rules );
		...
		if ( isset( $urls ) && count( $urls ) ) {
			foreach ( $urls as $url ) {
				...
				$mime_type              = $this->get_mime_type( Helper::get_file_extension_from_url( $url ) );
	...
	private function get_type( $url ) {
		$extension = strtolower( Helper::get_file_extension_from_url( $url ) );
