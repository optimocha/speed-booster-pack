...
class WpAdmin {
	...
	public function add_admin_bar_links( \WP_Admin_Bar $admin_bar ) {
		...
		if ( current_user_can( 'manage_options' ) ) {
			...
			$admin_bar->add_node( [
				...
				'meta'  => [
					'target' => '_self',
					'html'   => '<style>#wpadminbar #wp-admin-bar-speed_booster_pack .ab-item{background:url("' . SBP_URL . 'assets/admin/images/icon.svg?ver=' . SBP_VERSION . '") no-repeat 5px center;padding-left:25px;}#wpadminbar #wp-admin-bar-speed_booster_pack .ab-item:hover{color:white;}</style>',
				],
			] );
			...
			if ( Helper::getOption( 'localize_tracking_scripts' ) ) {
			...
			if ( Cloudflare::is_cloudflare_active() ) {
			...
			if ( Helper::getOption( 'sucuri_enable' ) ) {
	...
	public function set_notices() {
		...
		if ( $transient_value = get_transient( 'sbp_clear_sucuri_cache' ) ) {
			...
			NoticeManager::display_notice( 'sbp_clear_sucuri_cache',
		...
		NoticeManager::display_notice( 'sbp_notice_cloudflare',
			'<p><strong>' . SBP_PLUGIN_NAME . ':</strong> ' . $notice_message . '</p>',
			$notice_type,
			true,
			'flash' );
		...
		NoticeManager::display_notice( 'sbp_notice_cache',
		...
		if ( get_transient( 'sbp_notice_tracker_localizer' ) ) {
			NoticeManager::display_notice( 'sbp_notice_tracker_localizer',
		...
		if ( get_transient( 'sbp_advanced_cache_error' ) ) {
			NoticeManager::display_notice( 'sbp_advanced_cache_error',
		...
		if ( get_transient( 'sbp_wp_config_error' ) ) {
			NoticeManager::display_notice( 'sbp_wp_config_error',
		...
		NoticeManager::display_notice( 'sbp_warmup_started',
			...
			if ( current_user_can( 'manage_options' ) ) {
				...
				if ( ! $notice_meta ) {
					if ( isset( $notice['depends_on'] ) && $notice['depends_on'] ) {
						if ( NoticeManager::has_dismissed( $notice['depends_on'] ) ) {
					...
					if ( $notice_meta <= time() ) {
						NoticeManager::display_notice( $notice_key, '<p>' . $notice['text'] . '</p>', 'info', true, 'one_time', 'toplevel_page_sbp-settings' );
					}
				}
			}
		}
	...
	public function check_required_file_permissions() {
		...
		foreach ( $check_list as $key => $item ) {
			if ( $wp_filesystem->exists( $item ) ) {
				if ( ! Helper::sbpCheckFilePermissions( $item ) ) {
		...
		if ( count( $permission_errors ) ) {
			...
			NoticeManager::display_notice( 'permission_errors', $notice_content, 'warning', false, 'recurrent', 'toplevel_page_sbp-settings' );
		}
	...
	public function upgrade_php_notice() {
		...
		NoticeManager::display_notice( 'upgrade_php_notice', '<p><strong>' . SBP_PLUGIN_NAME . '</strong>: ' .  __( 'You are using a really old PHP version! In a few months, Speed Booster Pack will stop working with PHP versions below 7.0, so we highly recommend you update PHP to the latest version (or ask your hosting company to do it).', 'speed-booster-pack' ) . '</p>', 'warning', true );
