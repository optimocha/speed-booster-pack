...
class Cloudflare  {
	...
	public static function update_cloudflare_settings( $saved_data ) {
		...
		foreach ( $cf_options as $option ) {
			if ( Helper::getOption( $option ) != $saved_data[ $option ] ) {
	...
	public static function send_request( $action, $method = 'GET', $post_fields = [], $override_credentials = [] ) {
		$email   = Helper::getOption( 'cloudflare_email' );
		$api_key = Helper::getOption( 'cloudflare_api' );
		$zone    = Helper::getOption( 'cloudflare_zone' );
	...
	public function sbp_get_cloudflare_settings() {
		if ( isset( $_GET['action'] ) && $_GET['action'] == 'sbp_get_cloudflare_settings' && current_user_can( 'manage_options' ) ) {
			// Check if empty
			if ( ! Helper::getOption( 'cloudflare_email' ) || ! Helper::getOption( 'cloudflare_api' ) || ! Helper::getOption( 'cloudflare_zone' ) ) {
				echo json_encode(
					[
						'status'  => 'empty_info',
	...
	public static function is_cloudflare_active() {
		return Helper::getOption( 'cloudflare_enable' ) && Helper::getOption( 'cloudflare_email' ) && Helper::getOption( 'cloudflare_api' ) && Helper::getOption( 'cloudflare_zone' );
	}
}
