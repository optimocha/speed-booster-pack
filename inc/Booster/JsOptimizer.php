 ...
 */
class JsOptimizer extends AbstractModule{
	/**
	...
	public function __construct() {
		...
		$this->js_optimize_strategy = Helper::getOption( 'js_optimize', 'off' );
		$this->js_footer            = Helper::getOption( 'js_footer' );
	...
	public function optimize_scripts( $html ) {
		if ( ( ! Helper::getOption( 'module_assets' ) || ( $this->js_optimize_strategy == 'off' && ! $this->js_footer ) ) && ! is_singular() ) {
			return $html;
		}

		$this->js_footer_exclude_rules = array_merge( Helper::explodeLines( Helper::getOption( 'js_footer_exclude' ) ), $this->default_excludes );
		$this->exclude_rules           = array_merge( Helper::explodeLines( Helper::getOption( 'js_exclude' ) ), $this->default_excludes );
		$this->include_rules           = array_merge( Helper::explodeLines( Helper::getOption( 'js_include' ) ), $this->default_includes );

		if ( is_singular() ) {
			$js_optimization_status = Helper::sbpGetPostMeta( get_the_ID(), 'js_optimize', 'main_setting' );
			$js_footer_status       = Helper::sbpGetPostMeta( get_the_ID(), 'js_footer_status', 'main_setting' );

			if ( $js_optimization_status == 'off' || ( ! Helper::getOption( 'module_assets' ) && $js_optimization_status == 'main_setting' ) ) {
				$this->js_optimize_strategy = 'off';
			} elseif ( $js_optimization_status != 'main_setting' ) {
				$this->exclude_rules        = array_merge( Helper::explodeLines( Helper::sbpGetPostMeta( get_the_ID(), 'js_exclude' ) ), $this->default_excludes );
				$this->include_rules        = array_merge( Helper::explodeLines( Helper::sbpGetPostMeta( get_the_ID(), 'js_include' ) ), $this->default_includes );
				$this->js_optimize_strategy = Helper::sbpGetPostMeta( get_the_ID(), 'js_optimize' );
			}

			if ( $js_footer_status == 'off' || ( ! Helper::getOption( 'module_assets' ) && $js_footer_status == 'main_setting' ) ) {
				$this->js_footer = false;
			} elseif ( $js_footer_status == 'on' ) {
				$this->js_footer               = true;
				$this->js_footer_exclude_rules = array_merge( Helper::explodeLines( Helper::sbpGetPostMeta( get_the_ID(), 'js_footer_exclude' ) ), $this->default_excludes );
			}
		}
