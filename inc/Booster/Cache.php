...
class Cache extends BaseCache {
	...
	public static function clear_total_cache() {
		...
		Helper::sbpDeleteDirRecursively( SBP_CACHE_DIR );
		if ( Helper::getOption( 'caching_warmup_after_clear' ) && Helper::getOption( 'module_caching' ) ) {
	...
	public function handle_cache( $html ) {
		...
		$caching_expiry = Helper::getOption( 'caching_expiry' ) * HOUR_IN_SECONDS;
	...
	private function get_cache_file_path( $post_url = null, $is_mobile = false ) {
		...
		if ( ( wp_is_mobile() && Helper::getOption( 'caching_separate_mobile' ) ) || true === $is_mobile ) {
	...
	public static function set_wp_cache_constant( $wp_cache = true ) {
		...
		if ( Helper::sbpIsWpConfigWritable() ) {
			// get wp config as array
			$wp_config_file = Helper::sbpGetWpConfigPath();
	...
	public static function options_saved_listener( $saved_data ) {
		...
		if ( Helper::sbpShouldDisableFeature( 'caching' ) != false ) {
			return;
		}

		$module_caching_option = Helper::getOption( 'module_caching' );
		...
		if ( $saved_data['module_caching'] != $module_caching_option ) {
			...
			if ( ! Helper::sbpCheckFilePermissions( WP_CONTENT_DIR ) ) {
				set_transient( 'sbp_advanced_cache_error', 1 );

				return;
			}

			if ( file_exists( $advanced_cache_path ) && ! Helper::sbpCheckFilePermissions( $advanced_cache_path ) ) {
				set_transient( 'sbp_advanced_cache_error', 1 );
	...
	public static function options_saved_filter( $data ) {
		...
		$module_caching_option = Helper::getOption( 'module_caching', '0' );
...
# END Speed Booster Pack';
		...
		if ( $wp_filesystem->exists( $htaccess_file_path ) ) {
			...
			if ( ( isset( $saved_data['module_caching'] ) && $saved_data['module_caching'] ) || ( $saved_data == [] && Helper::getOption( 'module_caching' ) ) ) {
...
//		add_action( 'delete_term', 'SpeedBooster\SBP_Cache::clear_total_cache' );  // When a term is deleted.
	...
	private function check_query_strings() {
		...
		if ( ! empty( $_GET ) ) {
			// Get included rules
			$include_query_strings = Helper::explodeLines( Helper::getOption( 'caching_include_query_strings' ) );
