	...
	public function add_admin_bar_links( \WP_Admin_Bar $admin_bar ) {
		...
		if ( current_user_can( 'manage_options' ) ) {
			...
			$admin_bar->add_node( [
				...
				'meta'  => [
					'target' => '_self',
					'html'   => '<style>#wpadminbar #wp-admin-bar-speed_booster_pack .ab-item{background:url("' . SPEED_BOOSTER_PACK['url'] . 'admin/images/icon.svg?ver=' . SPEED_BOOSTER_PACK['version'] . '") no-repeat 5px center;padding-left:25px;}#wpadminbar #wp-admin-bar-speed_booster_pack .ab-item:hover{color:white;}</style>',
				],
			] );
