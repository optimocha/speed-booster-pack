...
class JSOptimizer
{
    ...
    private function add_defer_attribute()
    {
        foreach ($this->included_scripts as $script) {
            if (str_ireplace([' async', ' defer', 'data-noptimize="1"', 'data-cfasync="false"', 'data-pagespeed-no-defer'], '', $script) === $script) {
    ...
    private function convert_inline_to_base64()
    {
        foreach ($this->changed_scripts as &$script) {
            ...
            if (isset($matches[2]) && str_replace(['data-noptimize="1"', 'data-cfasync="false"', 'data-pagespeed-no-defer'], '', $matches[0]) === $matches[0]) {
