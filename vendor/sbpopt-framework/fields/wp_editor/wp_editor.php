 ...
 *
 */
if ( ! class_exists( 'SBPOPT_Field_wp_editor' ) ) {
  class SBPOPT_Field_wp_editor extends SBPOPT_Fields {
    ...
    public function render() {
      ...
      echo ( sbpopt_wp_editor_api() ) ? '<div class="sbpopt-wp-editor" data-editor-settings="'. esc_attr( json_encode( $editor_settings ) ) .'">' : '';

      echo '<textarea name="'. esc_attr( $this->field_name() ) .'"'. $this->field_attributes( $attributes ) . $editor_height .'>'. $this->value .'</textarea>';

      echo ( sbpopt_wp_editor_api() ) ? '</div>' : '';
    ...
    public function enqueue() {
      ...
      if ( sbpopt_wp_editor_api() && function_exists( 'wp_enqueue_editor' ) ) {
    ...
    public function setup_wp_editor_media_buttons() {
      ...
      echo 'var sbpopt_media_buttons = '. json_encode( $media_buttons ) .';';
      echo '</script>';
    ...
    public function setup_wp_editor_settings() {
      ...
      if ( sbpopt_wp_editor_api() && class_exists( '_WP_Editors') ) {

        $defaults = apply_filters( 'sbpopt_wp_editor', array(
          'tinymce' => array(
            'wp_skip_init' => true
          ),
        ) );

        $setup = _WP_Editors::parse_settings( 'sbpopt_wp_editor', $defaults );

        _WP_Editors::editor_settings( 'sbpopt_wp_editor', $setup );
