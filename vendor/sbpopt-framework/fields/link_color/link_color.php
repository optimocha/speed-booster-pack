 ...
 *
 */
if ( ! class_exists( 'SBPOPT_Field_link_color' ) ) {
  class SBPOPT_Field_link_color extends SBPOPT_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'sbpopt' ),
        'hover'    => esc_html__( 'Hover', 'sbpopt' ),
        'active'   => esc_html__( 'Active', 'sbpopt' ),
        'visited'  => esc_html__( 'Visited', 'sbpopt' ),
        'focus'    => esc_html__( 'Focus', 'sbpopt' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="sbpopt--left sbpopt-field-color">';
          echo '<div class="sbpopt--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="sbpopt-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
