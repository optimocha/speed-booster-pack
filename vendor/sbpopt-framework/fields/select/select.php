 ...
 *
 */
if ( ! class_exists( 'SBPOPT_Field_select' ) ) {
  class SBPOPT_Field_select extends SBPOPT_Fields {
    ...
    public function render() {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        if ( ! empty( $args['ajax'] ) ) {
          ...
          $args['settings']['data']['nonce'] = wp_create_nonce( 'sbpopt_chosen_ajax_nonce' );
        ...
        $chosen_sortable  = ( $args['chosen'] && $args['sortable'] ) ? ' sbpopt-chosen-sortable' : '';
        $chosen_ajax      = ( $args['chosen'] && $args['ajax'] ) ? ' sbpopt-chosen-ajax' : '';
        $placeholder_attr = ( $args['chosen'] && $args['placeholder'] ) ? ' data-placeholder="'. esc_attr( $args['placeholder'] ) .'"' : '';
        $field_class      = ( $args['chosen'] ) ? ' class="sbpopt-chosen'. esc_attr( $chosen_rtl . $chosen_sortable . $chosen_ajax ) .'"' : '';
        ...
        if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
          ...
          if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
            ...
            echo '<select name="'. $field_name .'" class="sbpopt-hide-select hidden"'. $multiple_attr . $field_attr .'>';
