 ...
 *
 */
if ( ! class_exists( 'SBPOPT_Taxonomy_Options' ) ) {
  class SBPOPT_Taxonomy_Options extends SBPOPT_Abstract{
    ...
    public function render_taxonomy_form_fields( $term ) {
      ...
      $errors    = ( ! empty( $term_id ) ) ? get_term_meta( $term_id, '_sbpopt_errors_'. $this->unique, true ) : array();
      $errors    = ( ! empty( $errors ) ) ? $errors : array();
      $class     = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_term_meta( $term_id, '_sbpopt_errors_'. $this->unique );
      }

      wp_nonce_field( 'sbpopt_taxonomy_nonce', 'sbpopt_taxonomy_nonce'. $this->unique );

      echo '<div class="sbpopt sbpopt-taxonomy sbpopt-show-all sbpopt-onload sbpopt-taxonomy-'. esc_attr( $classname ) .'-fields '. esc_attr( $class ) .'">';
        ...
        if ( $taxonomy === $this->taxonomy ) {
          ...
          if ( ! empty( $section['fields'] ) ) {
            foreach ( $section['fields'] as $field ) {
              ...
              SBPOPT::field( $field, $this->get_meta_value( $field, $term_id ), $this->unique, 'taxonomy' );
    ...
    public function save_taxonomy( $term_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'sbpopt_taxonomy_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';
      $taxonomy = ( ! empty( $_POST[ 'taxonomy' ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ 'taxonomy' ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'sbpopt_taxonomy_nonce' ) ) {
        return $term_id;
      }
      ...
      $data = apply_filters( "sbpopt_{$this->unique}_save", $data, $term_id, $this );

      do_action( "sbpopt_{$this->unique}_save_before", $data, $term_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_term_meta( $term_id, '_sbpopt_errors_'. $this->unique, $errors );
        }

      }

      do_action( "sbpopt_{$this->unique}_saved", $data, $term_id, $this );

      do_action( "sbpopt_{$this->unique}_save_after", $data, $term_id, $this );
