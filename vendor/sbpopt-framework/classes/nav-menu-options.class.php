 ...
 *
 */
if ( ! class_exists( 'SBPOPT_Nav_Menu_Options' ) ) {
  class SBPOPT_Nav_Menu_Options extends SBPOPT_Abstract{
    ...
    public function wp_edit_nav_menu_walker( $class, $menu_id ) {
      ...
      if( version_compare( $wp_version, '5.4.0', '<' ) ) {
        ...
        if ( ! class_exists( 'SBPOPT_Walker_Nav_Menu_Edit' ) ) {
          SBPOPT::include_plugin_file( 'functions/walker.php' );
        }

        return 'SBPOPT_Walker_Nav_Menu_Edit';
    ...
    public function wp_nav_menu_item_custom_fields( $menu_item_id, $item, $depth, $args ) {
      ...
      $errors = ( ! empty( $menu_item_id ) ) ? get_post_meta( $menu_item_id, '_sbpopt_errors_'. $this->unique, true ) : array();
      $errors = ( ! empty( $errors ) ) ? $errors : array();
      $class  = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_post_meta( $menu_item_id, '_sbpopt_errors_'. $this->unique );
      }

      echo '<div class="sbpopt sbpopt-nav-menu-options'. esc_attr( $class ) .'">';

        foreach ( $this->sections as $section ) {

          $section_icon  = ( ! empty( $section['icon'] ) ) ? '<i class="sbpopt-nav-menu-icon '. esc_attr( $section['icon'] ) .'"></i>' : '';
          $section_title = ( ! empty( $section['title'] ) ) ? $section['title'] : '';

          echo '<div class="sbpopt-fields">';

          echo ( $section_title || $section_icon ) ? '<div class="sbpopt-nav-menu-title"><h4>'. $section_icon . $section_title .'</h4></div>' : '';
          ...
          if ( ! empty( $section['fields'] ) ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              SBPOPT::field( $field, $this->get_meta_value( $menu_item_id, $field ), $this->unique .'['. $menu_item_id .']', 'menu' );
    ...
    public function wp_update_nav_menu_item( $menu_id, $menu_item_db_id, $menu_item_args ) {
      ...
      $data = apply_filters( "sbpopt_{$this->unique}_save", $data, $menu_item_db_id, $this );

      do_action( "sbpopt_{$this->unique}_save_before", $data, $menu_item_db_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_post_meta( $menu_item_db_id, '_sbpopt_errors_'. $this->unique, $errors );
        }

      }

      do_action( "sbpopt_{$this->unique}_saved", $data, $menu_item_db_id, $this );

      do_action( "sbpopt_{$this->unique}_save_after", $data, $menu_item_db_id, $this );
