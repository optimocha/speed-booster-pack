 ...
 *
 */
if ( ! class_exists( 'SBPOPT_Profile_Options' ) ) {
  class SBPOPT_Profile_Options extends SBPOPT_Abstract{
    ...
    public function render_profile_form_fields( $profileuser ) {
      ...
      $errors     = ( ! empty( $profile_id ) ) ? get_user_meta( $profile_id, '_sbpopt_errors_'. $this->unique, true ) : array();
      $errors     = ( ! empty( $errors ) ) ? $errors : array();
      $class      = ( $this->args['class'] ) ? ''. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_user_meta( $profile_id, '_sbpopt_errors_'. $this->unique );
      }

      echo '<div class="sbpopt sbpopt-profile-options sbpopt-onload'. esc_attr( $class ) .'">';

      wp_nonce_field( 'sbpopt_profile_nonce', 'sbpopt_profile_nonce'. $this->unique );
      ...
      foreach ( $this->sections as $section ) {
        ...
        if ( ! empty( $section['fields'] ) ) {
          ...
          foreach ( $section['fields'] as $field ) {
            ...
            SBPOPT::field( $field, $this->get_meta_value( $profile_id, $field ), $this->unique, 'profile' );
    ...
    public function save_profile( $user_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'sbpopt_profile_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'sbpopt_profile_nonce' ) ) {
        return $user_id;
      }
      ...
      $data = apply_filters( "sbpopt_{$this->unique}_save", $data, $user_id, $this );

      do_action( "sbpopt_{$this->unique}_save_before", $data, $user_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_user_meta( $user_id, '_sbpopt_errors_'. $this->unique, $errors );
        }

      }

      do_action( "sbpopt_{$this->unique}_saved", $data, $user_id, $this );

      do_action( "sbpopt_{$this->unique}_save_after", $data, $user_id, $this );
